  ...
  const deleteCountry = country => {
    fetch(API_URL + "/countries/" + country, {
      ...
      .then(response => {
        ...
        setCountriesList(
          countriesList.filter(element => element.country.id !== country)
        );
      })
      .catch(err => console.error(err));
  };
              ...
              <tr key={country.country.id}>
                ...
                <td>
                  {country.cities.map(city => {
                    return (
                      <table key={city.id}>
                        <tr className="cities-list">
                          {city.name}
                          <div>
                            <Link to={"/city/" + city.id}>
                              <i className="show-city-icon fa-solid fa-eye"></i>
                            </Link>
                            &nbsp;
                            <Link to={"/admin/update_city/" + city.id}>
                              <i className="update-city-icon fa-solid fa-pen"></i>
                            </Link>
                            &nbsp;
                            <i
                              className="delete-city-icon fa-solid fa-trash-can"
                              onClick={() => {
                                if (
                                  window.confirm(
                                    " 🚫 Are you sure you want to delete this City (it's forever) ? 🚫 "
                                  )
                                ) {
                                  deleteCity(city.id);
                                }
                  ...
                  <div className="delete-country">
                    <i
                      ...
                      onClick={() => {
                        if (
                          window.confirm(
                            " 🚫 Are you sure you want to delete this Country (it's forever) ? 🚫 "
                          )
                        ) {
                    ...
                    &nbsp;
                    <i
                      className="update-city-icon fa-solid fa-pen"
                      onClick={() => {
                        setEditCountry(true);
