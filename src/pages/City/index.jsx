                    ...
                    <table className="rates">
                        <tbody>
                            <tr>
                                ...
                                <td className='progress'><ProgressBar width={(city.activities * 100 / 5)}>{city.activities}/5</ProgressBar></td>
                            </tr>
                            <tr>
                                <td>💵 Cost</td>
                                <td>{city.cost > 2500? "🥵": null}</td>
                                <td className='progress'><ProgressBarCost width={(city.cost * 100 / 5000)}>{city.cost > 2500 ? <>{city.cost}<span>$/mo</span> </> : " " }</ProgressBarCost>{city.cost <= 2500 ? <p className="cost-message"> {city.cost}<span>$/mo</span> </p> : null}</td>
                            </tr>
                            <tr>
                                <td>💻 Workplaces</td>
                                <td>{city.works_places < 3? "😟" : null}</td>
                                <td className='progress'><ProgressBar width={(city.works_places * 100 / 5)}>{city.works_places}/5</ProgressBar></td>
                            </tr>
                            <tr>
                                <td>🚑 Healthcare</td>
                                <td>{city.healthcare < 3? "🤕": null}</td>
                                <td className='progress'><ProgressBar width={(city.healthcare * 100 / 5)}>{city.healthcare}/5</ProgressBar></td>
                            </tr>

                            <tr>
                                <td>👌 Safety</td>
                                <td>{city.safety < 3? "😨" : null}</td>
                                <td className='progress'><ProgressBar width={(city.safety * 100 / 5)}>{city.safety}/5</ProgressBar></td>
                            </tr>
                            <tr>
                                <td>📡 Internet</td>
